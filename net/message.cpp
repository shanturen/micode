...
int msg_handle_task::execute(worker *wkr)
{
	...
	LOG_INFO_VA("start msg task, msg id:%d, sn:%llu-%llu", _msg->get_id(), _msg->get_head().sn_high, _msg->get_head().sn_low);
	...
	//	LOG_FATAL_VA("close event in task %x", _msg->get_socket_event());
